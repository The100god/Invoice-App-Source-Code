...
interface SelectMeterialValidateProps {
  ...
  setNewMaterialError: React.Dispatch<
    React.SetStateAction<SelectMeterialValidateErrors[][]>
  >;
}
...
export const selectMaterialValidate = ({
  ...
  if (!activeFormData.selectedItem.trim() && !activeFormData.materialLink) {
    newErrors.selectedItem =
      "Selecting a item or adding link of material is required.";
    isValid = false;
  } else {
    if (
      [
